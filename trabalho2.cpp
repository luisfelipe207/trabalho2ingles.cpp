...
void setup()
{
    // Configurando os pinos 0 a 7 como entradas e 8 a 12 como saídas
    pinMode(0, INPUT);
    pinMode(1, INPUT);
    pinMode(2, INPUT);
    pinMode(3, INPUT);
...
{
    // Função para somar dois bits com o carryBit
    int bitResult = 0; 
    int aux1, aux2 = 0;
    if ((b1a ^ b2a) ^ cBit)
    {
...
{
    // Função para calcular o carryBit
    int aux1, aux2 = 0; 
...
void loop()
{
    ...
    if (soma == 1)
    {
        carryBit = 0; // Inicializa carryBit como 0
    ...
    // Escreve os resultados nos pinos de saída correspondentes
